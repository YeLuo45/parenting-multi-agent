...
function detectIntent(
	question: string,
): "hazard" | "first_aid" | "triage" | "checklist" | "category" | "general" {
	const q = question.toLowerCase();
	if (
		/(急救|first.aid|cpr|心肺复苏|止血|海姆立克|heimlich|choking|burn.*怎么处理)/i.test(
			q,
		)
	)
		return "first_aid";
	if (/(检查|checklist|清单|全部|所有|有哪些|list)/i.test(q))
		return "checklist";
	if (
		/(窒息|choking|中毒|poison|烫伤|burn|溺水|drown|触电|跌倒|fall|绳.*勒|中暑|car.*hot|车内|安全座椅|枪|firearm|gun|插座|电线|楼梯|换尿布|意外|风险)/i.test(
			q,
		)
	)
		return "hazard";
	if (/(紧急|emergency|911|120|triage|严重|严不严重|怎么办|怎么办)/i.test(q))
		return "triage";
	return "general";
}
...
export function detectFirstAidTopic(
	question: string,
): FirstAidTopic | undefined {
...
function formatFirstAid(topic: FirstAidTopic, isInfant: boolean): string {
	...
	for (const step of guide.steps) {
		const durPart = step.durationSeconds
...
function formatChecklist(ageMonths: number): string {
	...
	const lines = [
		`🛡️ ${Math.floor(ageMonths)} 月龄安全检查清单（共 ${all.length} 项，🔴 高风险 ${critical.length} 项）：`,
		"",
	];
...
export class SafetyGuardAgent implements Agent {
	...
	async respond(
		...
		if (intent === "hazard") {
			...
			const formatted = hazards
				.map((h) => formatHazard(h))
				.join("\n\n---\n\n");
			return {
				agentId: this.id,
				agentName: this.name,
				content: `${formatted}\n\n${SAFETY_DISCLAIMER}`,
				confidence: 0.85,
				urgency: hazards.some((h) => h.severity === "high")
					? "high"
					: "info",
			};
		}
