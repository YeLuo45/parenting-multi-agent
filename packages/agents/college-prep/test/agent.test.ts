...
describe("Knowledge: matchTopic", () => {
	it("matches standardized_test", () =>
		expect(matchTopic("SAT 备考")).toBe("standardized_test"));
	it("matches essay", () => expect(matchTopic("文书写作")).toBe("essay"));
	it("matches extracurricular", () =>
		expect(matchTopic("社团活动")).toBe("extracurricular"));
	it("matches application", () =>
		expect(matchTopic("ED申请")).toBe("application"));
	it("matches financial_aid", () =>
		expect(matchTopic("FAFSA")).toBe("financial_aid"));
	it("matches selection", () => expect(matchTopic("选校")).toBe("selection"));
	it("matches academics", () =>
		expect(matchTopic("GPA 提升")).toBe("academics"));
...
describe("CollegePrepAgent — factory and stage undefined", () => {
	...
	it("computes stage when child.stage is undefined", async () => {
		...
		const r = await createCollegePrepAgent().respond("SAT", child, {
			memory: undefined as any,
		});
		expect(r.agentId).toBe("college-prep");
	});
