...
describe("Knowledge: matchTopic", () => {
	it("matches exploration", () =>
		expect(matchTopic("职业探索")).toBe("exploration"));
	it("matches internship", () =>
		expect(matchTopic("实习机会")).toBe("internship"));
	it("matches resume", () => expect(matchTopic("简历写作")).toBe("resume"));
	it("matches interview", () =>
		expect(matchTopic("面试准备")).toBe("interview"));
	it("matches networking", () =>
		expect(matchTopic("LinkedIn人脉")).toBe("networking"));
	it("matches skills", () => expect(matchTopic("学什么技能")).toBe("skills"));
	it("matches first_job", () =>
		expect(matchTopic("第一份工作")).toBe("first_job"));
...
describe("CareerAgent — factory and stage undefined", () => {
	...
	it("computes stage when child.stage is undefined", async () => {
		...
		const r = await createCareerAgent().respond("实习", child, {
			memory: undefined as any,
		});
		expect(r.agentId).toBe("career");
	});
