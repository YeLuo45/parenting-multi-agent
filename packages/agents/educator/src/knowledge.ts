...
export function getEduStage(months: number): EduStageInfo | null {
	return (
		EDU_STAGES.find(
			(s) => months >= s.monthsRange.min && months < s.monthsRange.max,
		) ?? null
	);
}
...
export const LEARNING_STYLES: LearningStylePattern[] = [
	...
	},
	{
		...
		nameEn: "Auditory",
		patterns: [
			/(听觉|auditory|听|声音|读出来|讲故事|讨论|对话|music|音乐)/i,
		],
		characteristics: [
			"记得名字不记得脸",
			"喜欢讨论",
	...
	},
	{
		...
		nameEn: "Kinesthetic",
		patterns: [
			/(动觉|kinesthetic|动手|做|实验|运动|操作|tactile|play|hand)/i,
		],
...
export const INTERESTS: InterestInfo[] = [
	{
		...
		nameEn: "STEM",
		patterns: [
			/(科学|数学|编程|stem|工程|物理|化学|生物|robot|lego|积木|数学|math|code|coding|编程)/i,
		],
		activities: [
			"乐高/积木",
			"科学实验",
	...
	},
	{
		...
		nameEn: "Sports",
		patterns: [
			/(运动|球|游泳|跑步|体育|sport|football|basketball|soccer|swim|run|ball|跳|平衡车)/i,
		],
	...
	},
	{
		...
		nameEn: "Language",
		patterns: [
			/(阅读|读书|绘本|英语|英文|中文|讲故事|read|book|story|language|外语)/i,
		],
		activities: [
			"亲子阅读",
			"分级读物",
	...
	},
	{
		...
		nameEn: "Music",
		patterns: [
			/(音乐|钢琴|唱歌|儿歌|乐器|music|piano|sing|instrument|guitar|drum|打击乐)/i,
		],
		activities: [
			"听音乐",
			"唱歌",
	...
	},
	{
		...
		nameEn: "Social",
		patterns: [
			/(社交|朋友|合群|分享|合作|social|friend|play.date|teamwork|团队)/i,
		],
		activities: [
			"角色扮演",
			"小组游戏",
...
export function detectInterests(text: string): InterestCategory[] {
	...
	for (const i of INTERESTS) {
		for (const re of i.patterns) {
			if (re.test(text) && !found.includes(i.category))
				found.push(i.category);
		}
	}
	return found;
}
