...
function detectIntent(question: string): "habit" | "screen" | "sleep" | "streak" | "list" | "general" {
	const q = question.toLowerCase();
	// List intent takes priority when query contains list keywords
	if (/(列表|有什么|全部|所有|browse)/i.test(q)) return "list";
	if (/(屏幕|screen.time|看电视|看手机|ipad|tablet|看视频)/i.test(q)) return "screen";
	...
	if (/(习惯|habit|作息|routine|刷牙|洗澡|固定|培养|形成)/i.test(q)) return "habit";
...
function formatHabit(h: Habit): string {
	...
	for (const step of h.steps) {
		...
		lines.push(`  ${step.order}.${cuePart} ${step.action} (${step.durationMinutes}分钟)${rewardPart}`);
	}
	return lines.join("\n");
}
...
function formatHabitLoop(h: Habit): string {
	...
	const lines = [`🔁 ${h.title} 的习惯循环:`, "", `触发 (Cue): ${loop.cue}`, "", "流程 (Routine):"];
...
function formatStreak(days: number): string {
	...
	const levelLabel = { new: "新手期", building: "养成期", established: "稳定期", automatic: "自动化" }[level];
	const advice =
		level === "new"
			? "刚开始很正常，继续保持每天执行"
			: level === "building"
...
export class HabitBuilderAgent implements Agent {
	...
	async respond(question: string, child: ChildProfile, _context: AgentContext): Promise<AgentReply> {
		...
		if (intent === "list") {
			...
			const lines = [`📚 ${Math.floor(ageMonths)} 月龄推荐习惯 (${habits.length} 个)：`];
		...
		if (intent === "habit") {
			...
			const lines = [`💡 ${Math.floor(ageMonths)} 月龄适合培养的习惯：\n`];
			for (const h of habits) {
				lines.push(`- **${h.title}** (${h.domain}, ${h.formationDays}天形成周期)`);
			}
			lines.push(`\n输入习惯名查看详细步骤，例如："刷牙习惯"`);
			return {
				agentId: this.id,
