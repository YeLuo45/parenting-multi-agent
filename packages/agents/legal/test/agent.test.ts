...
describe("Knowledge: matchTopic", () => {
	...
	it("matches immigration", () => expect(matchTopic("移民签证")).toBe("immigration"));
	it("matches school_law", () => expect(matchTopic("休学")).toBe("school_law"));
...
describe("LegalAgent — topic queries", () => {
	...
	it("returns custody tips", async () => {
		const r = await agent.respond("监护权", makeChild(365 * 5, "preschool"), ctx);
		expect(r.content).toMatch(/监护|抚养/);
	});
	it("returns support tips", async () => {
		const r = await agent.respond("抚养费", makeChild(365 * 5, "preschool"), ctx);
		expect(r.content).toMatch(/抚养费|support/);
	});
	it("returns adoption tips", async () => {
		const r = await agent.respond("收养", makeChild(365 * 2, "toddler"), ctx);
		expect(r.content).toMatch(/收养|adoption/);
	});
	it("returns immigration tips", async () => {
		const r = await agent.respond("移民签证", makeChild(365 * 10, "school_age"), ctx);
		expect(r.content).toMatch(/移民|签证|immigration/);
	});
	it("returns school_law tips", async () => {
		const r = await agent.respond("休学", makeChild(365 * 12, "tween"), ctx);
		expect(r.content).toMatch(/学校|休学|school.law/);
	});
	it("returns wills tips", async () => {
		const r = await agent.respond("遗嘱", makeChild(365 * 5, "preschool"), ctx);
		expect(r.content).toMatch(/遗嘱|监护人/);
	});
	it("returns rights tips", async () => {
		const r = await agent.respond("儿童权利", makeChild(365 * 8, "school_age"), ctx);
...
describe("LegalAgent — factory and stage undefined", () => {
	it("computes stage when undefined", async () => {
		...
		const r = await createLegalAgent().respond("监护", child, { memory: undefined as any });
