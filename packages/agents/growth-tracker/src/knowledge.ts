...
export interface GrowthStandardRow {
	ageMonths: number;
	male: Record<GrowthMetric, { p3: number; p15: number; p50: number; p85: number; p97: number }>;
	female: Record<GrowthMetric, { p3: number; p15: number; p50: number; p85: number; p97: number }>;
}
...
export const GROWTH_STANDARDS: GrowthStandardRow[] = [
	{
		...
		male: {
			...
			head_circumference: { p3: 31.9, p15: 33.2, p50: 34.5, p85: 35.7, p97: 36.9 },
		},
		female: {
			height: { p3: 45.4, p15: 47.3, p50: 49.1, p85: 51.0, p97: 52.9 },
			weight: { p3: 2.4, p15: 2.7, p50: 3.2, p85: 3.8, p97: 4.2 },
			head_circumference: { p3: 31.5, p15: 32.7, p50: 33.9, p85: 35.1, p97: 36.2 },
		},
	},
	{
		ageMonths: 6,
		male: {
			...
			head_circumference: { p3: 40.6, p15: 41.9, p50: 43.3, p85: 44.7, p97: 46.0 },
		},
		female: {
			height: { p3: 60.1, p15: 62.3, p50: 64.1, p85: 66.2, p97: 68.4 },
			weight: { p3: 5.7, p15: 6.5, p50: 7.3, p85: 8.4, p97: 9.3 },
			head_circumference: { p3: 39.6, p15: 40.9, p50: 42.2, p85: 43.5, p97: 44.8 },
		},
	},
	{
		ageMonths: 12,
		male: {
			...
			head_circumference: { p3: 43.0, p15: 44.5, p50: 46.1, p85: 47.5, p97: 48.9 },
		},
		female: {
			height: { p3: 68.0, p15: 70.0, p50: 72.0, p85: 74.5, p97: 76.5 },
			weight: { p3: 7.0, p15: 7.9, p50: 8.9, p85: 10.1, p97: 11.3 },
			head_circumference: { p3: 42.0, p15: 43.5, p50: 45.0, p85: 46.5, p97: 47.8 },
		},
	},
	{
		ageMonths: 24,
		male: {
			...
			head_circumference: { p3: 45.0, p15: 46.5, p50: 48.0, p85: 49.5, p97: 51.0 },
		},
		female: {
			height: { p3: 79.6, p15: 82.0, p50: 85.0, p85: 88.0, p97: 91.0 },
			weight: { p3: 9.0, p15: 10.0, p50: 11.5, p85: 13.2, p97: 14.7 },
			head_circumference: { p3: 44.0, p15: 45.5, p50: 47.0, p85: 48.5, p97: 50.0 },
		},
	},
	{
		ageMonths: 36,
		male: {
			...
			head_circumference: { p3: 46.0, p15: 47.3, p50: 49.0, p85: 50.5, p97: 52.0 },
		},
		female: {
			height: { p3: 87.4, p15: 90.0, p50: 93.0, p85: 96.0, p97: 99.0 },
			weight: { p3: 10.8, p15: 12.1, p50: 13.9, p85: 16.0, p97: 17.8 },
			head_circumference: { p3: 45.0, p15: 46.5, p50: 48.0, p85: 49.7, p97: 51.0 },
		},
	},
	{
		ageMonths: 60,
		male: {
			height: { p3: 100.7, p15: 103.5, p50: 107.0, p85: 110.5, p97: 114.0 },
			weight: { p3: 14.1, p15: 15.9, p50: 18.3, p85: 21.1, p97: 23.7 },
			head_circumference: { p3: 47.5, p15: 48.7, p50: 50.5, p85: 52.0, p97: 53.5 },
		},
		female: {
			height: { p3: 99.4, p15: 102.5, p50: 106.0, p85: 109.5, p97: 113.0 },
			weight: { p3: 13.7, p15: 15.4, p50: 17.9, p85: 20.8, p97: 23.5 },
			head_circumference: { p3: 46.5, p15: 47.8, p50: 49.5, p85: 51.0, p97: 52.5 },
		},
	},
];
...
export function getPercentiles(
	...
	const lower =
		GROWTH_STANDARDS.reduce<GrowthStandardRow>(
...
export function estimatePercentile(value: number, ageMonths: number, metric: GrowthMetric, sex: GrowthSex): number {
...
export function classifyPercentile(percentile: number): "low" | "below_avg" | "average" | "above_avg" | "high" {
...
export function detectGrowthConcern(
	current: { metric: GrowthMetric; value: number; ageMonths: number; sex: GrowthSex },
	previous?: { metric: GrowthMetric; value: number; ageMonths: number; sex: GrowthSex },
): { concern: boolean; reason?: string } {
	const p = estimatePercentile(current.value, current.ageMonths, current.metric, current.sex);
	if (p <= 3) {
		return { concern: true, reason: `百分位低于 P3 (${p})，需就医评估` };
	}
	if (p >= 97) {
		return { concern: true, reason: `百分位高于 P97 (${p})，需医生评估是否过快` };
	}
	if (previous) {
		const prevP = estimatePercentile(previous.value, previous.ageMonths, previous.metric, previous.sex);
		if (Math.abs(p - prevP) > 25) {
			return { concern: true, reason: `百分位从 P${prevP} 变化到 P${p}，跨越 ≥2 条主要曲线，需就医评估` };
		}
	}
...
export const MILESTONES: Milestone[] = [
	// 0-3 months
	{ ageMonthsMin: 0, ageMonthsMax: 3, domain: "gross_motor", description: "俯卧时能抬头", redFlag: false },
	{ ageMonthsMin: 0, ageMonthsMax: 3, domain: "fine_motor", description: "手能触碰嘴", redFlag: false },
	{ ageMonthsMin: 0, ageMonthsMax: 3, domain: "social", description: "对父母微笑", redFlag: false },
	{ ageMonthsMin: 0, ageMonthsMax: 3, domain: "language", description: "对声音有反应", redFlag: false },
	{ ageMonthsMin: 0, ageMonthsMax: 3, domain: "cognitive", description: "视线追随物体", redFlag: false },
	// 4-6 months
	{ ageMonthsMin: 4, ageMonthsMax: 6, domain: "gross_motor", description: "能翻身", redFlag: false },
	{ ageMonthsMin: 4, ageMonthsMax: 6, domain: "fine_motor", description: "主动抓握物体", redFlag: false },
	{ ageMonthsMin: 4, ageMonthsMax: 6, domain: "language", description: "咿呀学语", redFlag: false },
	// 7-9 months
	{ ageMonthsMin: 7, ageMonthsMax: 9, domain: "gross_motor", description: "能坐稳", redFlag: false },
	{ ageMonthsMin: 7, ageMonthsMax: 9, domain: "language", description: "能听懂简单指令", redFlag: false },
	{ ageMonthsMin: 7, ageMonthsMax: 9, domain: "social", description: "认生", redFlag: false },
	// 10-12 months
	{ ageMonthsMin: 10, ageMonthsMax: 12, domain: "gross_motor", description: "扶站", redFlag: false },
	{ ageMonthsMin: 10, ageMonthsMax: 12, domain: "fine_motor", description: "拇指食指对捏", redFlag: false },
	{ ageMonthsMin: 10, ageMonthsMax: 12, domain: "language", description: "能叫 '爸爸/妈妈'", redFlag: false },
	// 13-18 months
	{ ageMonthsMin: 13, ageMonthsMax: 18, domain: "gross_motor", description: "独立行走", redFlag: false },
	{ ageMonthsMin: 13, ageMonthsMax: 18, domain: "language", description: "能说 10+ 词", redFlag: false },
	// 18-24 months
	{ ageMonthsMin: 18, ageMonthsMax: 24, domain: "language", description: "能说两词短句", redFlag: false },
	{ ageMonthsMin: 18, ageMonthsMax: 24, domain: "social", description: "模仿大人行为", redFlag: false },
	// RED FLAGS (developmental warning signs)
	{ ageMonthsMin: 4, ageMonthsMax: 6, domain: "gross_motor", description: "6 月龄仍不能抬头", redFlag: true },
	{ ageMonthsMin: 0, ageMonthsMax: 6, domain: "social", description: "6 月龄仍不对人微笑", redFlag: true },
	{ ageMonthsMin: 16, ageMonthsMax: 24, domain: "language", description: "18 月龄仍不会说单词", redFlag: true },
	{ ageMonthsMin: 16, ageMonthsMax: 24, domain: "social", description: "2 岁仍不会模仿", redFlag: true },
];

/** Get milestones for a specific age. */
export function getMilestonesForAge(ageMonths: number): { expected: Milestone[]; redFlags: Milestone[] } {
...
export function classifyBMI(bmi: number, ageMonths: number): "underweight" | "normal" | "overweight" | "obese" {
...
export function weightGainVelocity(
	...
	if (previousWeight === undefined || previousAgeDays === undefined) return null;
