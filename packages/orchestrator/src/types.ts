...
export interface MemoryLayerLike {
	startSession(childId: string, context?: Record<string, unknown>): import("@parenting/memory").Session;
	getSession(sessionId: string): import("@parenting/memory").Session | null;
	updateSession(sessionId: string, context: Record<string, unknown>): import("@parenting/memory").Session | null;
	addEpisode(childId: string, type: import("@parenting/memory").EpisodeType, content: Record<string, unknown>): import("@parenting/memory").Episode;
	getEpisodes(childId: string, type?: import("@parenting/memory").EpisodeType, limit?: number): import("@parenting/memory").Episode[];
...
export type OrchestratorEvent =
	| { type: "ask_started"; question: string; childId: string; sessionId?: string; at: number }
	| { type: "l0_escalation"; childId: string; redFlag: RedFlag; sessionId?: string; at: number }
	| { type: "routing"; childId: string; matchedAgentIds: string[]; sessionId?: string; at: number }
	| {
			type: "agent_reply";
			childId: string;
			reply: AgentReply;
	...
	| { type: "ask_completed"; childId: string; replies: AgentReply[]; sessionId?: string; at: number };
...
export function detectTopics(question: string): AgentTopic[] {
	...
	const topicMap: Array<[AgentTopic, RegExp]> = [
		...
		["knowledge", /(知识|科普|文章|资料|搜索|搜|找|查询|查|看看|读|看看|查阅|知识库|百科|科学|科普|原理|为什么|怎么形成的|原因|evidence|knowledge|fact|article|reference)/i],
		["growth", /(身高|体重|长高|长重|身高体重|成长曲线|percentile|百分位|生长|发育曲线|head.circumference|头围|bmi|体重曲线|height|weight|grow|growth.chart)/i],
		["habits", /(习惯|作息|规律|schedule|routine|habit|刷牙|洗脸|洗澡|睡觉时间|起床|固定|定时|train.habit|form.habit|戒|屏|屏幕时间|screen.time)/i],
		["safety", /(安全|危险|事故|急救|first.aid|受伤|流血|中毒|烫伤|触电|溺水|吞|异物|窒息|choke|safe|hazard|emergency|poison|burn)/i],
		["social", /(社交|朋友|同伴|playmate|peer|play.date|一起玩|交朋友|内向|外向|害羞|认生|陌生人|share|cooperate|teamwork|团体|合作|分享|轮流|take.turn)/i],
	];
