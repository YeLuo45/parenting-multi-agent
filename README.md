...
- `npm run build:web` may print Vite warnings about Node modules from `better-sqlite3` being externalized for browser compatibility. The build is accepted when the command exits with code 0; browser runtime uses `WebMemoryLayer` rather than SQLite.
...
- The CI e2e step uses `continue-on-error: true` because the production bundle currently has a known React 19 production-mount issue (`r is not a function`). The component behaviour is fully covered by jsdom + RTL unit tests in `apps/web/test/*.test.tsx`, which the `test-coverage` job enforces.
- Fixing the React 19 mount issue and turning e2e into a hard gate is tracked separately.
